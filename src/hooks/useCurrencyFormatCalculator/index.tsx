...
import { basicCalculator, extractNumberFromMathematicExpression, 
  isMathematicOperation, removeLeadingZeroes, removeNonNumerics, removeNonMathematicOperation,
  isMathematicOperator, removeNonNumericsExceptDash, stringToCurrency } from '../../utils';
...
function useCurrencyFormatCalculator(initialValue:string = "000", options?:CurrencyFormatCalculatorProps): returnCurrencyCalculatorFormat {
  ...
  const onChange = useCallback((e: React.ChangeEvent<HTMLInputElement>): void => {
      ...
      if(['-','+','/','*', '='].includes(lastChar)) {
        if(isMathematicOperation(value.replace(/\s/g,''))) {
          ...
          const values = extractNumberFromMathematicExpression(value.slice(0, -1));
          if(values) {
            const total = basicCalculator(values.operation, values.firstValue, values.lastValue, precision);
            if(lastChar === '=') {
          ...
          if(lastChar === '=') {
            newValue = formatCurrency(removeNonNumericsExceptDash(onlyOperation), style);
            callbackChange(e.currentTarget, newValue);
          } else {
            ...
            lastTwoCharacter[0] : "";
            let valueWithoutOperator = onlyOperation.replaceAll(charToRemove, '').trim();
            valueWithoutOperator = valueWithoutOperator.charAt(0) === "-" ? 
            valueWithoutOperator.substring(1) : valueWithoutOperator;
            newValue = `${dash}${valueWithoutOperator.replaceAll(lastChar, '').trim()} ${lastChar} `;
          }
        }
      } else {
        const values = extractNumberFromMathematicExpression(value);
        if(values) {
          newValue = `${values.firstValue} ${values.operation} ${formatCurrency(removeNonNumerics(values.lastValue))}`;
        } else {
  ...
  const onKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>): void  => {
    ...
    if (["Delete", "Backspace"].includes(e.key)) {
      ...
      if(value.length > 1) {
        ...
        const operator = ['-','+','/','*'].find(op => valueWithoutSignal.includes(op));
        ...
        if(operator) {
          ...
          if(values) {
            ...
            if(removeNonNumerics(values.lastValue) == zeros) {
              ...
              newValue = `${values.firstValue} ${values.operation} ${formatCurrency(values.lastValue)}`;
            }
          } else {
          ...
          const zeros = Array.from({length: precision}, (_) => '0').join('');
          const removeDash = removeNonNumericsExceptDash(newCurrencyValue) === `-${zeros}` ?
          removeNonNumerics(newCurrencyValue): removeNonNumericsExceptDash(newCurrencyValue)
          newValue = formatCurrency(removeDash, style);
          callbackChange(e.currentTarget, newValue)
        }
      } else {
        const zerosFormatted = formatCurrency(Array.from({length: precision + 1}, (_) => '0').join(''), style);
  ...
  const formatCurrency = useCallback((rawVal: string, styleCurrency: styles = "decimal" ) => {
